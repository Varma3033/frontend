    ...
    >
      <div className="mx-auto max-w-7xl">
        <div className="mb-10 text-center">
          ...
          <p className="pb-4 text-black/50">
            {projectpage?.projectPageDesc || "Selected Work"}
          </p>
          <h1 className="text-4xl font-black uppercase md:text-5xl">
            {projectpage?.projectPageTitle || "Projects"}
          </h1>
        </div>
        ...
        <div className="space-y-10 md:space-y-20">
          ...
          {projects.map((project: any, index: number) => {
            ...
            return (
              <div
                key={project.id}
                className={`group flex flex-col gap-2 rounded-[2rem] border border-black/10
            ...
            hover:text-black hover:bg-white/60 hover:animate-ease transition duration-300 hover:-translate-y-1 hover:shadow-xl"
                  >
                    <p className="text-xl px-4 py-2">{project.viewMore}</p>
                  </Link>
                </div>
              </div>
            );
